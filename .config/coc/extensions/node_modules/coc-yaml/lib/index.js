...
const schema_extension_api_1 = __webpack_require__(32);
const paths_1 = __webpack_require__(33);
...
(function (VSCodeContentRequestRegistration) {
    ...
    VSCodeContentRequestRegistration.type = new vscode_languageserver_protocol_1.NotificationType('yaml/registerVSCodeContentRequest');
...
function activate(context) {
    ...
    const serverModule = context.asAbsolutePath(path_1.default.join('node_modules', 'yaml-language-server', 'out', 'server', 'src', 'server.js'));
    ...
    const clientOptions = {
        ...
        synchronize: {
            ...
            fileEvents: [coc_nvim_1.workspace.createFileSystemWatcher('**/*.?(e)y?(a)ml'), coc_nvim_1.workspace.createFileSystemWatcher('**/*.json')],
        },
    };
    // Create the language client and start it
    client = new coc_nvim_1.LanguageClient('yaml', 'YAML Support', serverOptions, clientOptions);
            ...
            if (Array.isArray(yamlValidation)) {
                yamlValidation.forEach((jv) => {
                    ...
                    if (Array.isArray(fileMatch) && typeof url === 'string') {
                        let uri = url;
                        if (uri[0] === '.' && uri[1] === '/') {
                            uri = paths_1.joinPath(coc_nvim_1.Uri.file(extension.extensionPath), uri).toString();
                        }
...
(function (SchemaModificationNotification) {
    // eslint-disable-next-line @typescript-eslint/ban-types
    SchemaModificationNotification.type = new vscode_languageserver_protocol_1.RequestType('json/schema/modify');
...
class SchemaExtensionAPI {
    ...
    requestCustomSchema(resource) {
        ...
        for (const customKey of Object.keys(this._customSchemaContributors)) {
            try {
                ...
                let uri;
                if (contributor.label && coc_nvim_1.workspace.textDocuments) {
                    const labelRegexp = new RegExp(contributor.label, 'g');
                    for (const doc of coc_nvim_1.workspace.textDocuments) {
    ...
    requestCustomSchemaContent(uri) {
        if (uri) {
            const _uri = coc_nvim_1.Uri.parse(uri);
            if (_uri.scheme &&
