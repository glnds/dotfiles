...
const engine = new engine_1.CfnLintEngine();
const config = coc_nvim_1.workspace.getConfiguration("cfnlint");
...
function didChangeConfiguration(event) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!event.affectsConfiguration('cfnlint'))
            return;
...
function activate(context) {
    return __awaiter(this, void 0, void 0, function* () {
        yield engine.init();
        context.subscriptions.push(coc_nvim_1.workspace.onDidOpenTextDocument(didOpenTextDocument), coc_nvim_1.workspace.onDidSaveTextDocument(didSaveTextDocument), coc_nvim_1.workspace.onDidChangeConfiguration(didChangeConfiguration), coc_nvim_1.events.on("BufEnter", (bufnr) => {
            if (!bufnr) {
                return;
            }
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.CfnLintEngine = void 0;
const lodash_isequal_1 = __importDefault(__webpack_require__(3));
const coc_nvim_1 = __webpack_require__(1);
const vscode_languageserver_protocol_1 = __webpack_require__(5);
const cross_spawn_1 = __webpack_require__(35);
class CfnLintEngine {
    constructor() {
        this.source = "cfn-lint";
    ...
    init() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const cocPreferences = coc_nvim_1.workspace.getConfiguration("coc.preferences");
            this.projectRootDir = yield coc_nvim_1.workspace.resolveRootFolder(coc_nvim_1.Uri.parse(coc_nvim_1.workspace.uri), cocPreferences.get("rootPatterns", []));
            this.outputLine(`Using ${this.projectRootDir} as the working directory to execute cfn-lint`);
        });
    }
    lint(document) {
        const pathToLint = coc_nvim_1.Uri.parse(document.uri).fsPath;
        ...
        const diagnostics = [];
        child.on("error", (err) => {
            const msg = "Error: unable to start cfn-lint. Is cfn-lint installed corectly?";
            this.outputLine(`${msg}\n${err.stack}`);
            diagnostics.push({
                range: {
        ...
        child.on("exit", (code) => {
            ...
            if (numResults !== numFilteredResults) {
                this.outputLine(`Filtered to ${numFilteredResults} results after applying \"cfnlint.ignoreRulesRegExps\"`);
            }
        ...
        child.on("close", () => {
            if (!lodash_isequal_1.default(this.latestDiagnostics, diagnostics)) {
    ...
    getConfig() {
        return coc_nvim_1.workspace.getConfiguration("cfnlint");
    }
    outputLine(message) {
    ...
    convertSeverity(level) {
        switch (level) {
            ...
            default:
                throw new Error(`Unsupported severity returned by cfn-lint: "${level}"`);
        }
    }
    isValidCloudFormation(text, path) {
        if (this.isStandardCloudFormation(text, path))
            return true;
        if (this.isCustomCloudFormation(text, path))
            return true;
        this.outputLine(`Skipped linting ${path} because (1) it doesn't look like a standard CloudFormation or SAM template; (2) none of the RegExps defined in the setting "cfnlint.detectCfnRegExps" matched`);
        return false;
    }
    isStandardCloudFormation(text, path) {
        ...
        if (containsVersion && containsResources) {
            this.outputLine(`Detected CloudFormation file ${path} because it contains "AWSTemplateFormatVersion:" and "Resources:"`);
            return true;
        }
        return false;
    }
    isCustomCloudFormation(text, path) {
        const cfnRegExps = this.getConfig().detectCfnRegExps;
        if (cfnRegExps && cfnRegExps.length) {
            for (const regExpStr of cfnRegExps) {
                const regExp = new RegExp(regExpStr, "i");
                if (regExp.test(text)) {
                    this.outputLine(`Detected CloudFormation file ${path} because RegExp /${regExpStr}/ in setting "cfnlint.detectCfnRegExps" matched`);
                    return true;
                }
            }
        }
    ...
    filterResults(results) {
        const ignoreRulesRegExps = this.getConfig().ignoreRulesRegExps;
        if (!ignoreRulesRegExps || !ignoreRulesRegExps.length) {
            // Skip filtering
            return results;
        }
        const regExps = ignoreRulesRegExps.map((regExpStr) => new RegExp(regExpStr, "i"));
...
exports.CfnLintEngine = CfnLintEngine;
...
const cp = __webpack_require__(36);
const parse = __webpack_require__(37);
const enoent = __webpack_require__(49);
...
const path = __webpack_require__(15);
const which = __webpack_require__(39);
...
const shebangCommand = __webpack_require__(47);
...
const shebangRegex = __webpack_require__(48);
